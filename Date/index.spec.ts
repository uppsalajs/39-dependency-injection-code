...
describe("Date", () => {
	...
	describe("parse", () => {
		it.each([
			...
			const parsed = Date.parse(dateString, locale as any)
		...
		it.each([
			...
			["2023年06月15日", "zh-CN"],
			["2023년 06월 15일", "ko-KR"],
		] as const)("parse('%s', '%s') round trip", (date, locale) =>
			expect(Date.parse(date, locale)?.toString("ISO-8601")).toBe("2023-06-15")
		)
		it("parse with default locale", () => {
			const parsed = Date.parse("2023-06-15")
			expect(parsed).toBeDefined()
		...
		it.each([
			...
			expect(Date.parse(dateString, locale as any)).toBeUndefined()
		)
	})
})
